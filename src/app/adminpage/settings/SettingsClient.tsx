          ...
          <div className="flex items-center justify-between border-b border-border pb-4">
            ...
            {config.googleSheetsWebhookUrl ? (
              <div className="space-y-1 text-right">
                <span className="bg-green-500/10 text-green-500 border border-green-500/30 px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4" /> {t.settings.linked || "مربوط وتلقائي"}
                </span>
                <p className="text-xs text-muted font-mono font-bold mt-1 dir-ltr text-right">
                  📧 {config.googleSheetsWebhookUrl}
                </p>
              </div>
            ) : (
