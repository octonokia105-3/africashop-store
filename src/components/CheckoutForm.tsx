...
export default function CheckoutForm({ product }: { product?: Product }) {
  const [selectedOffer, setSelectedOffer] = useState<1 | 2 | 3>(1);
  const [addBump, setAddBump] = useState(false);
  const [state, formAction, isPending] = useActionState(submitOrder, null);

  // Generate dynamic offers if product is provided, otherwise fallback to default
  const defaultPrice = product ? product.price : 220;
  const originalPrice = product ? product.compare_at_price : 450;

  const offers = product ? [
    { id: 1, title: `1x ${product.title}`, price: defaultPrice, originalPrice: originalPrice, popular: true, desc: "باك واحد" },
    { id: 2, title: `2x ${product.title}`, price: defaultPrice * 2 - 41, originalPrice: originalPrice ? originalPrice * 2 : null, popular: false, desc: "توفير 41 درهم" },
    { id: 3, title: `3x ${product.title}`, price: defaultPrice * 3 - 111, originalPrice: originalPrice ? originalPrice * 3 : null, popular: false, desc: "أحسن ثمن!" },
  ] : [
    { id: 1, title: "باك واحد (7 قطع أركان)", price: 220, originalPrice: 450, popular: false, desc: "المجموعة الكاملة للعناية" },
    { id: 2, title: "باكين (2 باكات عناية)", price: 399, originalPrice: 900, popular: true, desc: "توفير 41 درهم (الأكثر طلباً)" },
    { id: 3, title: "3 باكات (باقة العائلة)", price: 549, originalPrice: 1350, popular: false, desc: "توفير 111 درهم!" },
  ];
          ...
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6 flex flex-col justify-between">
              <div>
                ...
                {/* Order Bump Upsell */}
                <div className="mt-6 border-2 border-dashed border-gold/60 bg-gold/5 rounded-2xl p-4 transition-all hover:border-gold shadow-[0_0_15px_rgba(232,93,136,0.15)] relative overflow-hidden">
                  <div className="absolute -right-10 top-2 bg-urgent text-white text-[10px] font-black px-10 py-1 rotate-45 z-10">عـرض خاص</div>
                  <label className="flex items-start gap-4 cursor-pointer relative z-20">
                    <div className="shrink-0 w-16 h-16 relative rounded-xl overflow-hidden border border-gold/30 bg-white flex items-center justify-center">
                       <Image src="/images/products/photo_9_2026-07-26_16-02-54.jpg" alt="Pure Argan Oil Booster" fill className="object-cover" />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-black text-light text-gold-shine text-sm md:text-base">إضافة سيروم زيت الأركان النقي!</span>
                      </div>
                      <p className="text-xs text-muted leading-tight mb-2">
                        أضيفي زجاجة زيت الأركان المغربي النقي 100% لتغذية أطراف الشعر وتقوية الأظافر. فقط بـ <span className="font-bold text-gold text-sm">49 درهم</span> بدلاً من 120 درهم!
                      </p>
                    </div>
            ...
            <div className="space-y-5 flex flex-col justify-center">
              ...
              <button 
                type="submit"
                disabled={isPending}
                className={`w-full mt-6 text-white font-black text-xl py-5 rounded-2xl shadow-[0_10px_40px_rgba(232,93,136,0.3)] transition-all duration-300 flex items-center justify-center gap-3 ${
                  isPending 
                    ? 'bg-muted cursor-not-allowed opacity-70' 
                    : 'bg-gradient-to-r from-gold to-[#d4426f] hover:-translate-y-1 hover:shadow-[0_15px_50px_rgba(232,93,136,0.45)]'
                }`}
              >
                {isPending ? (
