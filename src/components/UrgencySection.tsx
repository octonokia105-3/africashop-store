...
export default function UrgencySection({ stock }: { stock?: number }) {
  const [timeLeft, setTimeLeft] = useState({ hours: 2, minutes: 47, seconds: 33 });
  const stockCount = stock !== undefined ? stock : 7;
      ...
      >
        <div className="space-y-2 text-center md:text-right">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-light leading-tight">
            تبقى فقط <span className="text-urgent">{stockCount}</span> مجموعات بسعر التخفيض (220 درهم)!
          </h2>
          <p className="text-muted text-sm font-medium">لا تفوّتي هذه الفرصة — التوصيل مجاني لجميع المدن المغربية</p>
        </div>
